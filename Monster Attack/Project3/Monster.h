...
public:
	...
	Monster(string _name, string _type, int health, int attack, int defence, int speed, Move m1, Move m2, Move m3, Move m4) {
		...
		setHealthTotal(health); setHealthCurrent(health);
		setAttackTotal(attack); setAttackCurrent(attack);
		setDefenceTotal(defence); setDefenceCurrent(defence);
		setSpeedTotal(speed); setSpeedCurrent(speed);
