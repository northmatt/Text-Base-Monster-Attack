...
void LoseScreen::InitScene() {
	LoadIntoString("youDied.txt", title);
	centerOfScreen = Game::shared_instance().buffer.GetCSBI().dwSize.X / 4.f;
}

void LoseScreen::UpdateScene() {
	Game::shared_instance().buffer.WriteBuffer(title, centerOfScreen - 32, 1);
...
void LoseScreen::UpdateSwitch() {
	Game::shared_instance().PlayMusic("06-the-nightmare-begins");
	selection = 0;
	Game::shared_instance().buffer.SetMaxCam({ 0, 0 }, { 1, 1 });
	Game::shared_instance().buffer.SetCamPos({1, 1});
}

void LoseScreen::LoadIntoString(string fileName, string& stringToEdit) {
	ifstream theFile;
