...
private:
	...
	int power, accuracy, cooldown, cooldownCurrent;
public:
	//set
	...
	//constructor
	Move() { name = "NULL", effect = "NULL", type = "NULL", power = 0; accuracy = 0; cooldown = 0; }
	Move(string _name, string _effect, string _type, int _power, int _accuracy, int _cooldown) {
		setName(_name); setEffect(_effect); setType(_type); setPower(_power); setAccuracy(_accuracy); setCooldown(_cooldown);
		setCooldownCurrent(1);
	}
};